...
"""
Vista GET /rema/resumen-estaciones?from=<ms>&to=<ms>&measurement=<nombre>
Devuelve resumen por estación (sample_count, min, max, avg) usando columnas precalculadas.
"""


def resumen_estaciones(request):
    if request.method != "GET":
        return HttpResponseBadRequest()
    try:
        from_ts = int(request.GET.get("from", 0))
        to_ts = int(request.GET.get("to", 0))
        measurement_name = request.GET.get("measurement", "").strip()
    except (ValueError, TypeError):
        return HttpResponseBadRequest()
    if not measurement_name:
        return HttpResponseBadRequest()
    try:
        start = datetime.fromtimestamp(from_ts / 1000.0)
        end = datetime.fromtimestamp(to_ts / 1000.0)
    ...
    stations_data = (
        ...
        .annotate(
            ...
            weighted_sum=Sum(F("avg_value") * F("length")),
        )
    )
    stations = []
    for row in stations_data:
        station = Station.objects.get(pk=row["station"])
        total = row["sample_count"] or 0
        avg_val = (
            round(row["weighted_sum"] / total, 2)
            if total and row["weighted_sum"] is not None
            else None
        )
        stations.append({
            "station_id": station.pk,
            "station_name": str(station.location),
            "sample_count": total,
